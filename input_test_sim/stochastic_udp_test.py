...
def build_sample(index, run_seed=42):
    odu = _ODU_ONTOLOGY[index % len(_ODU_ONTOLOGY)]
    ...
    turn_packet = {
        "shared": {
            ...
            "pendulum_summary": "Dominant dwell in %s with %.2f stability and %d crossings." % (primary, pend_preset["stability"], crossings),
        },
        "diviner": {
...
def send_payload(data):
    ...
    print("Sent to %s:%s (%s): %s%s" % (TARGET_HOST, target_port(), mode, label, site))
